<template>
  <div v-loading="loading" class="page-editor editor-wrapper">
    ...
    <div class="editor-side-bar border-R">
      <el-tabs v-model="activeSideBar" tab-position="left" style="height: 100%;">
        <template v-for="(item, index) in sidebarMenus">
          <el-tab-pane v-if="!item.pageMode || (item.pageMode && item.pageMode === pageMode)" :key="index" :name="item.value">
    ...
    <!--预览-->
    <previewPage v-if="showPreview" :page-data="projectData" :page-id="id" @closePreview="showPreview = false" @publishFn="publishFn" @saveFn="saveFn" />
    <!--我的图片-->
    <imageLibs />
  </div>
</template>
