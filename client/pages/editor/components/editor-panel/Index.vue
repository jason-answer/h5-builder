    ...
    <div class="editor-pane-inner">
      <div class="editor-main" :style="{transform: 'scale('+scale+')', width: projectData.width + 'px', height: projectData.height + 'px'}">
        <div id="canvas-panel" ref="canvas-panel" class="page-preview-wrapper" :style="getCommonStyle(activePage.commonStyle)">
          <!--页面组件列表展示-->
          <edit-shape v-for="item in activePage.elements" :key="item.uuid" :uuid="item.uuid" :default-style="item.commonStyle" :style="getCommonStyle(item.commonStyle)" :active="item.uuid === activeElementUUID" @handleElementClick="handleElementClick(item.uuid)" @resize="handleElementResize">
