<template>
  <div class="components-page-manage scrollbar-wrapper">
    ...
    <el-scrollbar style="height: 100%;" class="components-main-conten">
      ...
      <!--</div>-->
      <!--</div>-->
      <div v-for="(item, index) in projectData.pages" :key="item.uuid" class="page-item clearfix" :class="{active: activePageUUID === item.uuid}" @click="changeActivePage(item)">
        <div class="inline-block">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item>
                <div class="btn-item" @click="copyPage(item)">
                  <i class="el-icon-document-copy" /> 复制页面
                </div>
              </el-dropdown-item>
              <el-dropdown-item>
                <div class="btn-item" @click="addPage(item)">
                  <i class="el-icon-document-add" /> 新增页面
                </div>
              </el-dropdown-item>
              <el-dropdown-item>
                <div class="btn-item" @click="deletePage(item)">
                  <i class="el-icon-delete" /> 删除页面
                </div>
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
