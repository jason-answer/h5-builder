      ...
      <ul class="scrollbar-wrapper">
        <li v-for="(item, index) in componentsList" :key="index" class="clearfix paddingB30">
          ...
          <div v-if="item.components && item.components.length">
            <div v-for="(element,i) in item.components" :key="i" class="components-lib-item" @click="handleClick(element)">
              <div class="lib-item-img">
                <i :class="[element.icon]" />
              </div>
              <p class="lib-item-title">
                {{ element.title }}
              </p>
            </div>
          </div>
          <div v-else>
            <p class="gray text-center paddingT20">
              待完善...
            </p>
          </div>
        </li>
