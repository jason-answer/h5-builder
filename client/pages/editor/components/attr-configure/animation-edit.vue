            ...
            <el-collapse-item v-for="(item, index) in activeElement.animations" :key="index">
              <template slot="title">
                ...
                <div class="el-animate-title-type-wrapper">
                  <span class="el-animate-title-type" @click.stop.prevent="handleShowChooseAnimatePanel(index)">{{ item.type }} <i class="el-icon-caret-right size-mini" /> </span>
                </div>
                <span class="el-animate-title-btn" @click.stop.prevent="runAnimate(index)"><i class="el-icon-caret-right" /></span>
                <span class="el-animate-title-btn" @click.stop.prevent="handleDeleteAnimate"><i class="el-icon-delete" /></span>
              </template>
              <div class="el-animate-item">
                <div class="attr-item-edit-wrapper">
                  ...
                  <div class="col-2 attr-item-edit-input">
                    <el-input-number v-model="item.duration" size="mini" controls-position="right" :min="0" :step="0.1" />
                  </div>
                </div>
                <div class="attr-item-edit-wrapper">
                  <p class="attr-item-title">
      ...
      <el-tabs v-model="activeName">
        <el-tab-pane v-for="item in animateCssDatas" :key="item.label" :label="item.label" :name="item.label">
          <el-scrollbar class="animate-choose-item">
            <div v-for="(animate, index) in item.children" :key="index" class="animate-choose-item-inner" @mouseover="hoverPreviewAnimate = animate.value" @mouseleave="hoverPreviewAnimate = ''" @click="handleChooseAnimate(animate)">
              <span class="animate-choose-image" :style="{backgroundPosition: `${item.bg_X}px ${item.bg_Y}px`}" :class="[hoverPreviewAnimate === animate.value && animate.value + ' animated']" />
              <p>{{ animate.label }}</p>
            </div>
          </el-scrollbar>
        </el-tab-pane>
