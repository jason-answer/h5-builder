  ...
  <div class="clearfix my-page-list">
    ...
    <div class="page-content">
      <div class="my-page-nav-list">
        ...
        <div class="my-page-nav-item" :class="{active: searchParams.type === 'share'}" @click="doSearch('share')">
          参与作品({{ shareCount }})
        </div>
      </div>
      ...
      <ul class="page-item-wrapper">
        ...
        <li v-for="(item, index) in pageList" :key="index" class="page-item">
          <span v-if="!item.isPublish" class="unpublish">未发布</span>
          <div class="header-mask">
            <div class="details-btn" @click="showPreviewFn(item._id)">
              预览
            </div>
          </div>
          <div class="cover">
          ...
          <div class="page-item-data-pv border-T">
            ...
            <div class="btn-wrapper">
              <el-button type="text" size="mini" @click="copyPage(item._id)">
                复制
              </el-button>
            </div>
            <div class="btn-wrapper">
              <el-dropdown placement="top-start">
                <span class="el-dropdown-link">
                  <el-button type="text" size="mini">更多 <i class="el-icon-more-outline" /></el-button>
                </span>
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-if="!item.isPublish && searchParams.type === 'my'">
                    <div @click="publishPage(item._id, index)">
                      发布
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <div @click="setAsTemplate(item._id)">
                      复制为模板
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <div @click="showPageData(item._id)">
                      页面数据
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="searchParams.type === 'my'">
                    <div @click="showAddUser(item._id)">
                      添加成员
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="searchParams.type === 'my'">
                    <div @click="deletePage(item._id, index)">
                      删除
                    </div>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="searchParams.type === 'share'">
                    <div @click="deleteShareUserPage(item._id, index)">
                      移出我的参与
                    </div>
                  </el-dropdown-item>
                </el-dropdown-menu>
      ...
      <el-dialog title="添加成员" :visible.sync="dialogVisible" width="500px">
        <el-form class="" label-width="120px">
          <el-form-item label="用户名/姓名:">
            <el-select v-model="addUserForm.userIds" class="full-input-w" multiple filterable remote reserve-keyword placeholder="请输入用户名/姓名搜索" :remote-method="remoteMethod" :loading="addUserloading">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" size="small" @click="submitAddUser">确 定</el-button>
        </span>
      </el-dialog>
    </div>

    <!--预览-->
    <previewPage v-if="showPreview" :page-id="previewId" @closePreview="showPreview = false" />
  </div>
</template>
