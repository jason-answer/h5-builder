...
router.get('/list/search', async ctx => {
  const keywords = ctx.request.keywords;
...
router.post('/login', async ctx => {
  const data = ctx.request.body;
  const result = await Users.findOne({ username: data.username }).select('username password').exec();
  if (!result) {
    ctx.status = 202;
...
router.post('/:_id', async ctx => {
  ...
  const data = ctx.request.body;
  const result = await Users.update({ _id }, { $set: data }, {
    runValidators: true
  });
  ctx.body = result.select('username name email avatar type roles').exec();
});
