  ...
  <div class="engine-template-wrapper">
    ...
    <div class="swiper-container">
      <div class="swiper-wrapper">
        <div v-for="(page, i) in pageData.pages" :key="i" class="swiper-slide flat relative hidden" :style="getCommonStyle(page.commonStyle)">
          <!--页面内容区域-->
          <div class="quark-page-wrapper" :style="getCommonStyle({...page.commonStyle, top: pageTop, height: pageData.height, width: pageData.width, position: 'relative'}, scalingRatio)">
            <componentsTemplate v-for="(item, index) in page.elements" :key="index" :element="item" :style="getCommonStyle(item.commonStyle, scalingRatio)" @handleElementClick="handleElementClick" />
          </div>
        </div>
      </div>
